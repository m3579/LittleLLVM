/*
 * Token.cpp
 *
 * /TheCompilerFramework/Sources/src/Token.cpp
 *
 *       Author: Mihir Kasmalkar
 * Date created: Aug 28, 2015
 *
 *      Purpose: Represents a token in the source code.
 *
 *        Usage: Generated by Lexer and used by Parser to assemble AST
 *
 *        Notes:
 *
 */

#include <sstream>
#include <iostream>

#include "Token.hpp"

#define NULLTOKENWARNING "You should usually not use the no-arg constructor for Tokens"

namespace token
{
	Token::Token(int ln, int cn, std::string text, TokenType type, std::string errorMessage) :
		 lineNumber(ln), columnNumber(cn), text(text), type(type), errorMessage(errorMessage)
	{

	}

	Token::~Token()
	{

	}

	bool Token::isOfType(TokenType type)
	{
	    return this->type == type;
	}

	bool Token::isNotOfType(TokenType type)
	{
	    return this->type != type;
	}

	std::string Token::toString()
	{
		std::stringstream summary;
		summary << "Token: |" << text << "|";
		summary << "\n\tType: " << type;
		summary << "\n\tAt: " << lineNumber;
		summary << " on " << columnNumber;

		return summary.str();
	}

	Token::operator bool() const
	{
		return type != TTYPE_NOTOKEN;
	}

    Token& Token::operator=(Token token)
    {
        lineNumber = token.lineNumber;
        columnNumber = token.columnNumber;
        text = token.text;
        type = token.type;

        return *this;
    }

    int Token::getLineNumber()
	{
		return lineNumber;
	}

    int Token::getColumnNumber()
	{
		return columnNumber;
	}

    std::string Token::getText()
	{
		return text;
	}

    TokenType Token::getType()
	{
		return type;
	}

	std::string Token::getErrorMessage()
	{
	    return errorMessage;
	}

	void Token::print(std::string tabs)
	{
        std::stringstream output;
        output << tabs << "Token: " << text << "\n";
        output << tabs << "\tType: " << type << "\n";
        output << tabs << "\tLine number: " << lineNumber << "\n";
        output << tabs << "\tColumn Number: " << columnNumber << "\n";

        std::cout << output.str() << "\n";
    }
} /* namespace token */
